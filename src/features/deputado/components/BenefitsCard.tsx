...
export function BenefitsCard({ deputadoId, onOpenMoradiaModal }: BenefitsCardProps) {
  ...
  return (
    <div className="lg:col-span-1 bg-navy/40 backdrop-blur-xl border border-white/10 rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-8 space-y-8 flex flex-col group/beneficios shadow-2xl relative hover:z-50 transition-all duration-300 overflow-hidden">
      ...
      <div className="flex-1 space-y-4 relative z-10">
        {isLoading ? (
          ...
          <div className="space-y-3">
            <div className="p-3.5 bg-white/[0.03] rounded-[1.5rem] border border-white/5 hover:border-indigo-500/30 transition-all flex flex-wrap items-center justify-between gap-3 group/item shadow-sm">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 bg-indigo-500/10 rounded-xl flex items-center justify-center text-indigo-400 shrink-0 group-hover/item:scale-110 transition-transform"><DollarSign size={16} /></div>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Salário Bruto</span>
              </div>
              <span className="text-white font-black text-sm text-right ml-auto">{beneficiosCard?.salario_bruto || '—'}</span>
            </div>
            ...
            <div className="p-3.5 bg-white/[0.03] rounded-[1.5rem] border border-white/5 hover:border-blue-500/30 transition-all space-y-4 group/item shadow-sm">
              ...
              <div className="space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-lg text-right ml-auto leading-tight ${beneficiosCard?.imovel_funcional?.includes('Faz uso') ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shadow-sm shadow-emerald-500/10' : 'bg-white/5 text-slate-500 border border-white/5 opacity-60'}`}>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex flex-col items-end ml-auto">
                    <button
                      ...
                      className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-lg flex items-center gap-2 transition-all relative overflow-hidden group/btn text-right
                    ...
                    ${beneficiosCard?.auxilio_moradia_mensal
                          ...
                          : 'cursor-default'}`}>
                      <span>{beneficiosCard?.auxilio_moradia || 'Não informado'}</span>
                      {beneficiosCard?.auxilio_moradia_mensal && <Info size={12} className="opacity-60 group-hover/btn:scale-125 transition-transform shrink-0" />}
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className="p-3.5 bg-white/5 rounded-2xl border border-white/5 hover:border-purple-400/30 transition-all flex flex-wrap items-center justify-between gap-3 group/passport">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-purple-500/10 rounded-lg text-purple-400 shrink-0"><Globe size={14} /></div>
                <span className="text-xs font-bold text-slate-400 uppercase">Passaporte</span>
              </div>
              <span className={`text-[10px] font-black uppercase px-2.5 py-0.5 rounded-lg text-right ml-auto ${beneficiosCard?.passaporte_diplomatico?.includes('Possui') ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20 shadow-sm shadow-purple-500/10' : 'bg-white/5 text-slate-500 border border-white/5'}`}>
                {beneficiosCard?.passaporte_diplomatico || 'Não informado'}
              </span>
            </div>

            <div className="p-3.5 bg-white/5 rounded-2xl border border-white/5 hover:border-rose-400/30 transition-all flex flex-wrap items-center justify-between gap-3 group/mission">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-rose-500/10 rounded-lg text-rose-400 shrink-0 transition-all"><Plane size={14} /></div>
                <div className="flex items-center gap-2 group/tip relative">
                  <span className="text-xs font-bold text-slate-400 uppercase">Missões</span>
                  <HelpCircle size={10} className="text-slate-600 hover:text-rose-400 transition-colors cursor-help shrink-0" />
                </div>
              </div>
              <span className="text-white font-black text-sm ml-auto">{beneficiosCard?.viagens_missao || '0'}</span>
            </div>

            <div className="p-3.5 bg-white/5 rounded-2xl border border-white/5 hover:border-amber-400/30 transition-all flex flex-wrap items-center justify-between gap-3 group/office">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-amber-500/10 rounded-lg text-amber-400 shrink-0"><Users size={14} /></div>
                <span className="text-xs font-bold text-slate-400 uppercase">Gabinete</span>
              </div>
              <span className="text-white font-black text-[10px] text-right leading-tight ml-auto">
